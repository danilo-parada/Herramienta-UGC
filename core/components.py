...
def render_page_banner(
    ...
    """
    Renderiza un banner institucional genérico reutilizable para cualquier página.
    Utiliza el mismo estilo que render_irl_banner.
    
    Args:
        title: Título principal del banner
        subtitle: Subtítulo o descripción corta
        body: Texto descriptivo más largo
        chips: Lista de diccionarios con 'label' (iniciales) y 'title' (texto completo) para los chips
    """
    ...
    def chip_html(label: str, title: str) -> str:
        initials = label
        svg = f'''
    ...
    chips_content = ""
    if chips:
        chips_content = "".join([chip_html(c.get("label", ""), c.get("title", "")) for c in chips])
    ...
    <div class=\"tray-irl-banner\">
        <div class=\"tray-irl-banner__inner\">
            <div>
                ...
                <div class=\"tray-irl-banner__icons\">{chips_content}</div>
            </div>
        </div>
...
def render_irl_banner(
    ...
    render_page_banner(
        ...
        body=body,
        chips=[
            {"label": "TRL", "title": "Tecnología (TRL)"},
            {"label": "BRL", "title": "Negocio (BRL)"},
            {"label": "IP", "title": "Propiedad Intelectual (IPRL)"},
            {"label": "Tm", "title": "Equipo (TmRL)"},
            {"label": "FRL", "title": "Financiamiento (FRL)"},
            {"label": "CRL", "title": "Cliente/Mercado (CRL)"},
        ]
    )
