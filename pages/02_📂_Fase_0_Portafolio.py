...
try:
    ...
    st.warning(f"No se pudo cargar el tema: {e}")
...
def _build_template_excel(template_df: pd.DataFrame):
    ...
    for idx, line in enumerate(instructions, start=1):
        cell = ws_instructivo.cell(row=idx, column=1, value=line)
        if Alignment:
            cell.alignment = Alignment(wrap_text=True)
            
    ws_instructivo.column_dimensions['A'].width = 100
    
    # Hoja 2: Plantilla
    ws_plantilla = wb.create_sheet('Plantilla')
    
    for col_idx, col_name in enumerate(template_df.columns, start=1):
        cell = ws_plantilla.cell(row=1, column=col_idx, value=col_name)
        if Alignment is not None:
            cell.alignment = Alignment(wrap_text=True, vertical='center')
        ws_plantilla.column_dimensions[get_column_letter(col_idx)].width = max(18, len(col_name) + 4)
    ws_plantilla.freeze_panes = 'A2'
...
render_page_banner(
    title="Fase 0 - Portafolio y filtro inicial",
    subtitle="Carga, normaliza y evalua iniciativas antes de avanzar a la radiografia IRL.",
    body="Gestiona tu portafolio de proyectos de innovación. Carga datos masivamente, normaliza la información y genera un ranking preliminar para priorizar las iniciativas con mayor potencial.",
    chips=[
        {"label": "CM", "title": "Carga Masiva"},
        {"label": "NR", "title": "Normalización"},
        {"label": "RK", "title": "Ranking"},
    ]
)
...
score_tables = st.session_state.get('score_tables', {})
...
# ============================================================================
# INICIALIZAR PORTAFOLIO BASADO EN SESIÓN
# ============================================================================
# Solo cargamos de BD si hay datos cargados EN ESTA SESIÓN
# En primera carga, siempre está vacío hasta que usuario carga ejemplo o archivo

datos_cargados_sesion = (st.session_state.get('fase0_ejemplo_loaded', False) or 
                         st.session_state.get('datos_portafolio_sesion', False))

if datos_cargados_sesion:
    try:
        portafolio_df = utils.normalize_df(db.fetch_df())
    except Exception as e:
        portafolio_df = pd.DataFrame()
else:
    # Primera carga o después de resetear = VACÍO
    portafolio_df = pd.DataFrame()
...
# === BOTONES PRINCIPALES ===
col_ejemplo_fase0, col_analisis_fase0, col_reset_fase0 = st.columns(3)

with col_ejemplo_fase0:
    st.markdown("**📊 Cargar Ejemplo**")
    st.caption("Pre-llena portafolio de ejemplo")
    
    # Verificar si el ejemplo ya fue cargado
    ejemplo_ya_cargado_fase0 = st.session_state.get('fase0_ejemplo_loaded', False)
    
    if st.button("Cargar Ejemplo", use_container_width=True, type="primary", key="btn_ejemplo", disabled=ejemplo_ya_cargado_fase0):
        sample = utils.normalize_df(_sample_portafolio())
        db.replace_all(sample)
        for key in ['ranking', 'fase1_payload', 'fase0_result']:
            st.session_state.pop(key, None)
        
        # Marcar como ejemplo cargado en esta sesión
        st.session_state['fase0_ejemplo_loaded'] = True
        st.session_state['datos_portafolio_sesion'] = True
        
        st.toast("✅ Ejemplo cargado correctamente", icon="✅")
        st.rerun()
    
    # Mostrar mensaje si el ejemplo ya fue cargado
    if ejemplo_ya_cargado_fase0:
        st.info("✅ Ejemplo cargado - El botón está desactivado. Usa 'Resetear' si deseas cargar uno nuevo.", icon="ℹ️")

with col_analisis_fase0:
    st.markdown("**🔍 Generar Ranking**")
    st.caption("Calcula ranking de proyectos")
    
    # Verificar si hay datos cargados usando solo session_state
    datos_disponibles = (st.session_state.get('fase0_ejemplo_loaded', False) or 
                        st.session_state.get('datos_portafolio_sesion', False))
    
    # Verificar si el ranking ya fue generado
    ranking_generated = st.session_state.get('fase0_result') is not None
    
    if st.button('Generar Ranking', use_container_width=True, type="primary", key="btn_calcular", disabled=not datos_disponibles or ranking_generated):
        try:
            ...
            if df_eval.empty:
                ...
                st.toast("✅ Ranking generado correctamente", icon="✅")
                st.rerun()
        except Exception as e:
            st.error(f"❌ Error al generar ranking: {str(e)}")
    
    # Mostrar mensaje si el ranking ya fue generado
    if ranking_generated:
        st.info("✅ Análisis ya fue generado. Usa 'Resetear' para generar uno nuevo.", icon="ℹ️")
    
    # Mostrar mensaje si no hay datos
    elif not datos_disponibles:
        st.info("⚪ Carga datos de ejemplo primero para generar el ranking.", icon="ℹ️")

with col_reset_fase0:
    st.markdown("**🔄 Resetear**")
    st.caption("Limpia todo y comienza de nuevo")
    
    if st.button('Resetear', use_container_width=True, type="secondary", key="btn_reset_fase0"):
        # Limpiar session_state
        for key in list(st.session_state.keys()):
            if any(pattern in key.lower() for pattern in ['ranking', 'fase0', 'resultado', 'payload']):
                st.session_state.pop(key, None)
        
        # Limpiar base de datos
        db.replace_all(pd.DataFrame())
        
        st.toast("✅ Portafolio reseteado", icon="✅")
        st.rerun()

# === SECCIÓN DE DESCARGAS ===
st.markdown("---")
st.markdown("### 📥 Descargas")
st.caption("Plantilla de carga masiva de proyectos")

col_template = st.container()

with col_template:
    template_df = _portafolio_template()
    template_bytes = _build_template_excel(template_df)
    
    st.download_button(
        label="📊 Descargar Plantilla (con Instructivo)",
        data=template_bytes,
    ...
    if not HAS_OPENPYXL and uploaded_file.name.lower().endswith(('xlsx', 'xls')):
        ...
        try:
            ...
            if uploaded_file.name.lower().endswith('.csv'):
                ...
                try:
                    df_import = pd.read_excel(uploaded_file, sheet_name='Plantilla')
                except ValueError:
...
st.caption('Visualiza y edita manualmente los proyectos de tu portafolio. Los cambios se guardan automáticamente al usar "Generar Ranking".')

# Solo mostrar el editor si hay datos cargados (ejemplo o archivo subido)
if portafolio_df.empty:
    st.info('⚪ No hay proyectos cargados. Usa "Cargar Ejemplo" o sube un archivo para comenzar.')
else:
...
# Nota: La lógica de cálculo de ranking se encuentra en la sección "Gestión de Portafolio" arriba.
...
if resultado is not None and not resultado.empty:
    ...
    # Botón movido a la sección "Siguiente Paso" al final de la página
    ...
    # Sección para ir a siguiente fase
    st.markdown("---")
    st.markdown("### 📋 Siguiente Paso")
    
    fase1_page = next(Path("pages").glob("03_*_Fase_1_*.py"), None)
    resultado = st.session_state.get('fase0_result')
    ready = resultado is not None and not resultado.empty
    
    col_go_fase1, col_info = st.columns([1, 1.5])
    ...
    with col_go_fase1:
        if st.button(
            ...
            disabled=not ready,
            key="btn_ir_fase1_new"
        ):
            st.session_state['navigate_to_fase1'] = True
    
    # Ejecutar navegación si se marcó el flag
    if st.session_state.get('navigate_to_fase1', False):
        st.session_state['navigate_to_fase1'] = False  # Resetear flag
        if fase1_page:
            st.session_state['fase1_ready'] = True
            st.switch_page(str(fase1_page))
        else:
            st.warning("La página de Fase 1 aún no está disponible.")
    
    with col_info:
        if ready:
            st.success("✅ ¡Ranking generado correctamente! Ya puedes ir a Fase 1")
        else:
            st.info(
                "📊 Genera el ranking haciendo clic en 'Generar Ranking' arriba para continuar."
            )
